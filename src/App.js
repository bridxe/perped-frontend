...
function AppHeaderLinks({ HeaderLink, small, openSettings, clickCloseIcon }) {
    return ( <
        ...
        div className = "App-header-link-container" >
        <
        a href = "https://gmxio.gitbook.io/gmx/"
        target = "_blank"
        ...
        /div> {
            small && ( <
                div className = "App-header-link-container" > { /* eslint-disable-next-line */ } <
                a href = "#"
...
function AppHeaderUser({
    ...
    if (!active) {
        return ( <
            ...
            <
            HeaderLink activeClassName = "active"
            ...
            /div> {
                ...
                showConnectionOptions && ( <
                    ...
                    imgSrc = { connectWalletImg } > { small ? "Connect" : "Connect Wallet" } <
                    /ConnectWalletButton>
                )
            } <
            /div>
...
function FullApp() {
    ...
    const HeaderLink = ({ isHomeLink, className, exact, to, children }) => {
        ...
        if (isHome && !(isHomeLink && !isOnHomePage)) {
            return ( <
                div className = { cx("a", className, { active: isHomeLink }) }
                onClick = {
                    () => showRedirectModal(to) } > { children } <
                /div>
            );
        }
    ...
    return ( <
        ...
        <
        AppHeaderUser HeaderLink = { HeaderLink }
        disconnectAccountAndCloseSettings = { disconnectAccountAndCloseSettings }
        openSettings = { openSettings }
        ...
        <
        div className = {
            cx("App-header-link-container", "App-header-top", {
        ...
        div className = "App-header-menu-icon-block"
        onClick = {
            () => setIsDrawerVisible(!isDrawerVisible) } > {!isDrawerVisible && < RiMenuLine className = "App-header-menu-icon" / > } { isDrawerVisible && < FaTimes className = "App-header-menu-icon" / > } <
        /div> <
        ...
        disconnectAccountAndCloseSettings = { disconnectAccountAndCloseSettings }
        openSettings = { openSettings }
        small setActivatingConnector = { setActivatingConnector }
        walletModalVisible = { walletModalVisible }
        setWalletModalVisible = { setWalletModalVisible }
        showNetworkSelectorModal = { showNetworkSelectorModal }
        /> <
        ...
        AnimatePresence > {
            isDrawerVisible && ( <
                ...
                <
                AppHeaderLinks HeaderLink = { HeaderLink }
                small openSettings = { openSettings }
                clickCloseIcon = {
                    () => setIsDrawerVisible(false) }
                /> <
                /motion.div>
        ...
        /AnimatePresence> {
            ...
            !isHome && ( <
                ...
                <
                Buy savedSlippageAmount = { savedSlippageAmount }
                setPendingTxns = { setPendingTxns }
                connectWallet = { connectWallet }
                /> <
                /Route> <
                ...
                <
                Referrals pendingTxns = { pendingTxns }
                connectWallet = { connectWallet }
                setPendingTxns = { setPendingTxns }
                /> <
                /Route> <
        ...
        a href = "https://github.com/gmx-io/gmx-interface"
        target = "_blank"
        rel = "noopener noreferrer" >
        GMX front end <
        /a>, hosted and served on the distributed, peer - to - peer { " " } <
        ...
        <
        Checkbox isChecked = { showPnlAfterFees }
        ...
        div className = "Exchange-settings-row" >
        <
        Checkbox isChecked = { isPnlInLeverage }
        ...
        /div> {
            isDevelopment() && ( <
                div className = "Exchange-settings-row" >
                <
                Checkbox isChecked = { shouldDisableOrderValidation }
